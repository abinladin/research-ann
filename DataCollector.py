...
class DataCollector:
    ...
    def attempt_for_all_neuron_sets(self, case_number, attempt_number):
        for number_of_neurons in range(0, MAX_NUMBER_OF_NEURONS - 1):
            ...
            for cycle in range(1, NUMBER_OF_TRAINING_CYCLES):
                ...
                self.network_array[number_of_neurons].train(nn.XOR_gate_inputs, nn.XOR_gate_outputs)
            self.case[case_number, attempt_number] = self.network_array[number_of_neurons].feed_forward(nn.XOR_gate_inputs)[case_number]
    ...
    #TODO: write a set of methods to navigate the data stored in network_array
